---
title: 多智能体失败原因
description: 多 Agent 协作的挑战和解决方案
---
...
- MAS（Multi Agent System，多智能体系统）更适合于群体智能场景，也就是多agent做的事情基本一致，比如多个agent互相对抗（狼人杀），互相启发（共同审阅一篇论文）。
...
1. 分析了五种流行的 MAS 框架，涉及 150 多个任务，每个轨迹平均超过15,000行文本。提出多智能体系统失败分类法（**Multi-Agent System Failure Taxonomy，MAST**）将失败模式分为3类，总共14种范式：
...
- 采用Grounded Theory（GT，扎根理论）方法，通过理论抽样、开放式编码、持续比较分析、备忘录和理论化等步骤，迭代地收集和分析多智能体系统（MAS）的执行轨迹。
...
- 计算Kappa, Kappa值越大说明一致性越强，一般Kappa>0.8就认为标注标准清晰可靠。
    ...
    $$kappa = \frac{p_o - p_e}{1 - p_e} = 0.2857$$
...
- MAS的失败模式违反了核电站、航空管制等**高可靠性组织（HRO，High Reliability Organization）**的规则，如"不遵从角色规范" (FM-1.2) 违反了 "极端层级分化"，"未能请求澄清" (FM-2.2) 违反了 "尊重专业知识" 。
